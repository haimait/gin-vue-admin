  ...
  <div>
        ...
        <el-form-item label="分类名称" prop="name">
         <el-input v-model="searchInfo.name" placeholder="搜索条件" />
           ...
           <el-form-item label="是否启用" prop="enable">
            <el-select v-model="searchInfo.enable" clearable placeholder="请选择" @clear="()=>{searchInfo.enable=undefined}">
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="是否启用" prop="enable" width="120">
            ...
            {{ filterDict(scope.row.enable,boolOptions) }}
            </template>
        </el-table-column>
        <el-table-column align="left" label="操作" min-width="120">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateCategory2Func(scope.row)">变更</el-button>
      ...
      <el-scrollbar height="550px">
        <el-descriptions column="1" border>
                ...
                <el-descriptions-item label="是否启用">
                        {{ filterDict(formData.enable,boolOptions) }}
                </el-descriptions-item>
        </el-descriptions>
      </el-scrollbar>
    </el-dialog>
...
const getTableData = async() => {
  const table = await getCategory2List({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteCategory2ByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateCategory2Func = async(row) => {
    const res = await findCategory2({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteCategory2Func = async (row) => {
    const res = await deleteCategory2({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
